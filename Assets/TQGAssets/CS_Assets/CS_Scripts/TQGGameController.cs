...
//			round.text = Mathf.Clamp(roundCount, 0f, 15f).ToString ();
		...
		IEnumerator AskQuestion( bool animateQuestion )
		{
			...
			if ( isGameOver == false )
			{
				...
				if ( currentQuestion < questions.Length )
				{
					...
					if ( questions[currentQuestion].sound )    // If we have a question sound, play it.
					{
						// Play the question sound
						PlayQuestionSound();
						
						// Enable the button in the question, so that we can click it to play the sound again
						questionObject.GetComponent<Button>().enabled = true;
					}
					else if ( questions[currentQuestion].image )    // If we have a question image, display it. Otherwise, hide the image object
					{
...
//						timerBar.fillAmount = timeLeft / questions[currentQuestion].time;
		...
		IEnumerator GameOver(float delay)
		{
			...
			if ( gameOverCanvas )    
			{
				...
				if ( gameOverCanvas.Find("ScoreTexts/TextScore") )    gameOverCanvas.Find("ScoreTexts/TextScore").GetComponent<Text>().text = "PUNTOS LOGRADOS " + players[currentPlayer].score.ToString();
...
//				gameOverCanvas.Find("ScoreTexts/TextHighScore").GetComponent<Text>().text = "HIGH SCORE " + highScore.ToString();
		...
		IEnumerator Victory(float delay)
		{
			...
			if ( victoryCanvas )    
			{
				...
				if ( victoryCanvas.Find("ScoreTexts/TextScore"))
				{
					//Write the score text, if it exists
...
//					victoryCanvas.Find("ScoreTexts/TextHighScore").GetComponent<Text>().text = "HIGH SCORE " + highScore.ToString();
		...
		public string SaveXml()
		{
			...
			for ( index = 0; index < questions.Length ; index++ )
			{
				...
				xmlString += @"    <Bonus>100</Bonus>" + "\n";
				
				// The time allowed to answer this question
				xmlString += @"    <Time>8</Time>" + "\n";
		...
		public void PlayQuestionSound()
		{
			...
			if ( soundSourceTag != string.Empty && questions[currentQuestion].sound ) 
			{
				// Play the sound
